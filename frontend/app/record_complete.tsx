        ...
        <SafeAreaView style={styles.container}>
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.header}>
                    <Text style={styles.headerEmoji}>{isAiAchievement ? '🤖' : '🎉'}</Text>
                    <Text style={[styles.headerTitle, { color: theme }]}>{isAiAchievement ? '称号獲得！' : 'お疲れ様でした！'}</Text>
                    <Text style={styles.headerSubtitle}>{isAiAchievement ? 'AIとの絆が深まりました。' : '今日の努力は、明日の自信になる。'}</Text>
                </View>
                ...
                {newAchievement && (
                    <View style={[
                        ...
                        <View style={[styles.newIconCircle, { backgroundColor: activeColor }]}>
                ...
                <View style={[styles.messageCard, { backgroundColor: theme + '20' }]}>
                    <Text style={styles.messageEmoji}>{isAiAchievement ? '✨' : '💪'}</Text>
                    <View style={styles.messageTextWrapper}>
                        <Text style={[styles.messageTitle, { color: theme }]}>AIコーチからのエール</Text>
                        <Text style={styles.messageText}>
                            {isAiAchievement 
                                ? "AIトレーナーをいっぱい使うと、さらに特別な実績がまだ得られるかもしれませんよ！" 
                                : `${streakDays}日目の積み上げ完了！その調子です！`}
                        </Text>
                    </View>
                </View>
                ...
                >
                    <Text style={styles.homeButtonText}>{isAiAchievement ? 'チャットに戻る' : 'ホームへ戻る'}</Text>
                    <Ionicons name={isAiAchievement ? "chatbubbles" : "arrow-forward"} size={20} color={COLORS.white} />
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    newAchievementCard: { width: '100%', borderRadius: 25, padding: 30, alignItems: 'center', marginBottom: 20, borderWidth: 2, elevation: 10, position: 'relative' },
